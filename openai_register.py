...
class LuckMailInbox:
    ...
    def fetch_code(self, timeout_sec: int = 180, poll: float = 6.0,
                   exclude_codes: Optional[List[str]] = None) -> str:
        ...
        while time.monotonic() - start < timeout_sec:
            ...
            try:
                ...
                print(
                    f"[otp][luckmail] 轮询 #{attempt}, has_new_mail={has_new_mail}, token_code={getattr(token_result, 'verification_code', None)}, 目标: {self.address}")
                for code in codes:
                ...
                for mail in mails:
                    ...
                    if message_id:
                        try:
                            ...
                            print(
                                f"[otp][luckmail] 检查邮件详情: message_id={message_id}, detail_code={detail_code!r}, subject={getattr(detail, 'subject', '')!r}")
                            body2 = " ".join([
                                str(getattr(detail, "subject", "") or ""),
                                str(getattr(detail, "body_text", "") or ""),
                                str(getattr(detail, "body_html", "") or ""),
...
def get_email_and_code_fetcher(
    ...
    def _build_tempmail_bundle():
        ...
        def fetch_code(timeout_sec: int = 180, poll: float = 6.0,
                       exclude_codes: Optional[List[str]] = None) -> str | None:
            exclude = set(exclude_codes or [])
            start = time.monotonic()
            attempt = 0
            while time.monotonic() - start < timeout_sec:
...
def _parse_int_csv(raw: str, default) -> List[int]:
    values = []
...
def _resolve_sub2api_settings(args=None) -> Dict[str, Any]:
    base_url = str(
        (getattr(args, "sub2api_base_url", None) if args else None) or SUB2API_BASE_URL or "").strip().rstrip("/")
    admin_api_key = str(
        (getattr(args, "sub2api_admin_api_key", None) if args else None) or SUB2API_ADMIN_API_KEY or "").strip()
    ...
    group_ids_raw = (getattr(args, "sub2api_group_ids", None) if args else None) or os.getenv(
        "SUB2API_GROUP_IDS") or "2"
    auto_upload = bool(getattr(args, "sub2api_upload", False)) or _as_bool(os.getenv("AUTO_UPLOAD_SUB2API"))
    target_count = int(str((getattr(args, "sub2api_target_count", None) if args else None) or os.getenv(
        "SUB2API_TARGET_COUNT") or "0").strip() or "0")
    return {
        "base_url": base_url,
...
def submit_callback_url(callback_url: str, expected_state: str, code_verifier: str, redirect_uri: str,
                        session=None) -> str:
...
def _sub2api_request(method: str, url: str, settings: Dict[str, Any], *, params,
...
def _sub2api_list_accounts_by_privacy_mode(settings: Dict[str, Any], privacy_mode: str, page_size: int = 1000) -> List[
    Dict[str, Any]]:
    base_url = str(settings.get("base_url") or "").rstrip("/")
    if not base_url:
        return []
    ...
    while True:
        ...
        if resp.status_code == 401 and not settings.get("admin_api_key") and settings.get("email") and settings.get(
                "password"):
...
def _sub2api_delete_account(account_id: Any, settings: Dict[str, Any]) -> bool:
    ...
    resp = _do_request()
    if resp is not None and resp.status_code == 401 and not settings.get("admin_api_key") and settings.get(
            "email") and settings.get("password"):
        new_token = _sub2api_login(settings)
        if new_token:
            settings["bearer"] = new_token
            resp = _do_request()
...
def _clean_sub2api_failed_training_accounts(settings: Dict[str, Any], privacy_mode: str = "training_set_failed",
    ...
    print(
        f"[Sub2Api] 全量拉取 {len(accounts)} 个，本地精确匹配 privacy_mode={privacy_mode} 命中 {len(matched_accounts)} 个，跳过 {len(skipped_accounts)} 个")
    if skipped_accounts:
    ...
    if not delete:
        ...
        return {"fetched": len(accounts), "matched": len(matched_accounts), "deleted": 0, "failed": 0,
    ...
    print(
        f"[Sub2Api] 清理完成：全量拉取 {len(accounts)} 个，本地精确命中 {len(matched_accounts)} 个，删除成功 {deleted} 个，失败 {failed} 个")
    return {"fetched": len(accounts), "matched": len(matched_accounts), "deleted": deleted, "failed": failed,
            "accounts": matched_accounts}
...
class MiniPoolMaintainer:
    def __init__(self, base_url: str, token: str, target_type: str = "codex", used_percent_threshold: int = 95,
    ...
    def upload_token(self, filename: str, token_data: dict, proxy: str = "") -> bool:
        ...
        for attempt in range(3):
            try:
                resp = py_requests.post(_join_mgmt_url(self.base_url, "/management/auth-files"), files=files,
                                        headers=headers, timeout=30, verify=False, proxies=proxies)
    ...
    def fetch_auth_files(self, timeout: int = 15):
        resp = py_requests.get(_join_mgmt_url(self.base_url, "/management/auth-files"),
                               headers=_mgmt_headers(self.token), timeout=timeout)
    ...
    async def probe_and_clean_async(self, workers: int = 20, timeout: int = 10, retries: int = 1):
        ...
        async def probe_one(session, item):
            ...
            res = {"name": name, "auth_index": auth_index, "invalid_401": False, "invalid_used_percent": False,
                   "used_percent": None}
            if not auth_index:
            ...
            payload = {"authIndex": auth_index, "method": "GET", "url": "https://chatgpt.com/backend-api/wham/usage",
                       "header": header}
            for attempt in range(retries + 1):
                try:
                    async with semaphore:
                        async with session.post(_join_mgmt_url(self.base_url, "/management/api-call"),
            ...
            try:
                async with semaphore:
                    async with session.delete(
                            f"{_join_mgmt_url(self.base_url, '/management/auth-files')}?name={encoded}",
                            headers=_mgmt_headers(self.token), timeout=timeout) as resp:
                        text = await resp.text()
                        data = _safe_json(text)
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=client_timeout, trust_env=True) as session:
            ...
            delete_tasks = [asyncio.create_task(delete_one(session, r.get("name"))) for r in invalid_list if
                            r.get("name")]
            deleted_ok = 0
            deleted_fail = 0
    ...
    try:
        ...
        for login_attempt in range(3):
            try:
                ...
                auth_cookie = s2.cookies.get("oai-client-auth-session", domain=".auth.openai.com") or s2.cookies.get(
...
def main():
    ...
    parser.add_argument("--mail-provider", choices=["auto", "gptmail", "tempmail", "luckmail"], default="auto",
                        help="临时邮箱提供商：auto/luckmail/gptmail/tempmail")
    ...
    parser.add_argument("--luckmail-api-secret", default=os.getenv("LUCKMAIL_API_SECRET"),
                        help="LuckMail API Secret（可选）")
    parser.add_argument("--luckmail-use-hmac", action="store_true", help="LuckMail 使用 HMAC 鉴权")
    parser.add_argument("--luckmail-project-code", default=os.getenv("LUCKMAIL_PROJECT_CODE", "openai"),
                        help="LuckMail 项目编码")
    parser.add_argument("--luckmail-email-type", default=os.getenv("LUCKMAIL_EMAIL_TYPE", "ms_graph"),
                        help="LuckMail 邮箱类型")
    parser.add_argument("--luckmail-domain", default=os.getenv("LUCKMAIL_DOMAIN", "outlook.com"),
                        help="LuckMail 指定邮箱域名")
    parser.add_argument("--luckmail-order-timeout", type=int, default=int(os.getenv("LUCKMAIL_ORDER_TIMEOUT", "180")),
                        help="LuckMail 接码等待超时(秒)")
    parser.add_argument("--luckmail-poll-interval", type=float, default=float(os.getenv("LUCKMAIL_POLL_INTERVAL", "6")),
                        help="LuckMail 轮询间隔(秒)")

    parser.add_argument("--sub2api-base-url", default=os.getenv("SUB2API_BASE_URL"), help="Sub2API 基础地址")
    parser.add_argument("--sub2api-admin-api-key", default=os.getenv("SUB2API_ADMIN_API_KEY"),
                        help="Sub2API 管理端全局 API Key（优先使用）")
    parser.add_argument("--sub2api-bearer", default=os.getenv("SUB2API_BEARER"),
                        help="Sub2API Bearer token（兼容旧方式）")
    parser.add_argument("--sub2api-email", default=os.getenv("SUB2API_EMAIL"), help="Sub2API 管理员邮箱（旧登录方式）")
    parser.add_argument("--sub2api-password", default=os.getenv("SUB2API_PASSWORD"),
                        help="Sub2API 管理员密码（旧登录方式）")
    parser.add_argument("--sub2api-group-ids", default=os.getenv("SUB2API_GROUP_IDS", "2"),
                        help="Sub2API 绑定分组，逗号分隔")
    parser.add_argument("--sub2api-upload", action="store_true", help="注册成功后自动上传到 Sub2API")
    parser.add_argument("--sub2api-target-count", type=int, default=int(os.getenv("SUB2API_TARGET_COUNT", "0")),
                        help="Sub2API 目标可用账号数，达到后暂停注册；0 表示不限制")
    parser.add_argument("--sub2api-clean-training-set-failed", action="store_true",
                        help="清理 Sub2API 中 privacy_mode=training_set_failed 的账号")
    ...
    parser.add_argument("--prune-local", action="store_true",
                        help="上传成功后删除本地 token 文件与账号行（适用于 Sub2API / CPA）")
    ...
    while True:
        ...
        if res:
            ...
            need_sub2api_upload = bool(
                tokens and sub2api_settings.get("auto_upload") and sub2api_settings.get("base_url") and tokens.get(
                    "refresh_token"))
